...
// Coerce anything progress-shaped (including the trimmed
// {testId, answeredQuestionIds} objects other query hooks cache under a
// shared key) into a complete PracticeProgressLike, so callers never crash
// on a missing questionStats / incorrectQuestionIds.
function norm(progress: Partial<PracticeProgressLike> | null | undefined): PracticeProgressLike {
  return {
    answeredQuestionIds: progress?.answeredQuestionIds ?? [],
    incorrectQuestionIds: progress?.incorrectQuestionIds ?? [],
...
export function mergePracticeProgress(
  docs: (Partial<PracticeProgressLike> | null | undefined)[],
): PracticeProgressLike {
...
export function accuracyPct(progress: Partial<PracticeProgressLike> | null | undefined): number | null {
  const stats = Object.values(norm(progress).questionStats);
...
export function weakAreaCount(progress: Partial<PracticeProgressLike> | null | undefined): number {
  return Object.values(norm(progress).questionStats).filter((s) => s.attempts > 0 && s.correct / s.attempts < 0.5).length;
}
...
export function bankBuckets(
  progress: Partial<PracticeProgressLike> | null | undefined,
