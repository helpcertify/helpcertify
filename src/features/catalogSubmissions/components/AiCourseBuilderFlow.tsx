...
export function AiCourseBuilderFlow({ isAdmin = false }: { isAdmin?: boolean }) {
  ...
  const submitMutation = useMutation({
    ...
    onSuccess: (r) => {
      pushToast(
        isAdmin ? 'Published to the catalog.' : `Submitted for review: ${r.totalQuestions} question(s).`,
        'success'
      );
      resetAll();
    },
  ...
  return (
    <div className="rounded-xl border border-surface-border bg-surface-raised p-6">
      ...
      <p className="mb-4 text-sm text-ink-faint">
        Describe a course topic and let AI draft a module outline and the exam questions - review
        and edit everything before {isAdmin ? 'publishing' : 'submitting it for admin review'}.
      </p>
      ...
      {draftId && generatedQuestions && (
        <div>
          <h3 className="mb-2 font-medium text-ink">
            Review generated questions ({totalQuestions} total)
          </h3>
          {warnings.length > 0 && (
