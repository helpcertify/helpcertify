...
//
// Layout, top to bottom: brand header -> title/learner/exam name (unchanged
// from before) -> a compact results panel filling what used to be dead
// space in the middle -> a footer with the disclaimer on the left and the
// certificate ID + QR code (verification label) grouped together on the
// right. The results panel always shows Questions Completed, Attempt and
// Completed On; Score is a separate, distinctly-labeled cell that only
// appears when cert.scoreCorrect is actually available (never fabricated,
// and never merged into the completion figure - see the cell-count/spacing
// logic below for how the panel stays balanced with 3 or 4 cells). Neither
// this panel nor anything else on the certificate ever prints "Passed",
// "Certified" or "Exam Ready" - those would misrepresent a short practice
// session as a pass/fail credential, which this certificate is not (see the
// disclaimer).
async function buildCertificatePdf(cert: Record<string, any>, certificateId: string, verifyBaseUrl: string): Promise<Buffer> {
  ...
  doc.setFontSize(10);
  doc.setTextColor(faint);
  doc.text(`${cert.certificationName} preparation`, pageWidth / 2, 100, { align: 'center' });

  // --- Results panel (the old "middle area" was blank below the exam name
  // and above the footer - this fills it with a real stat panel instead of
  // one crowded sentence). Computed fresh from the raw counts rather than
  // trusting cert.completionPercent, which historically doubled as a score
  // percentage for a quiz certificate - completion and score are always
  // two distinct numbers here, each under its own label, so a 100%-answered
  // session is never printed in a way that could be misread as 100% correct.
  const completedDate = cert.completedAt?.toDate ? cert.completedAt.toDate() : new Date(cert.completedAt);
  const totalQuestions = Number(cert.totalQuestions) || 0;
  const questionsCompleted = Number(cert.questionsCompleted) || 0;
  const hasScore = cert.scoreCorrect !== null && cert.scoreCorrect !== undefined && totalQuestions > 0;
  const scorePercent = hasScore ? Math.round((Number(cert.scoreCorrect) / totalQuestions) * 100) : null;

  type PanelCell = { label: string; value: string };
  const cells: PanelCell[] = [
    { label: 'Questions Completed', value: totalQuestions > 0 ? `${questionsCompleted} / ${totalQuestions}` : String(questionsCompleted) },
  ];
  if (hasScore) {
    cells.push({ label: 'Score (Correct Answers)', value: `${cert.scoreCorrect} / ${totalQuestions} · ${scorePercent}%` });
  }
  cells.push({ label: 'Attempt', value: `#${cert.attemptNumber}` });
  cells.push({ label: 'Completed On', value: completedDate.toLocaleDateString(undefined, { day: 'numeric', month: 'short', year: 'numeric' }) });

  // Panel geometry - centred in the space between the exam-name block above
  // (ends ~104) and the footer below (starts ~148), balanced whether it's
  // showing 3 cells (no score available) or 4 (score available): columns
  // are always evenly divided across the same panel width, so an absent
  // Score cell never leaves a lopsided gap.
  const panelX = 30;
  const panelY = 108;
  const panelW = pageWidth - panelX * 2;
  const panelH = 30;
  doc.setFillColor(panelBg);
  doc.setDrawColor(panelBorder);
  doc.setLineWidth(0.3);
  doc.roundedRect(panelX, panelY, panelW, panelH, 2, 2, 'FD');

  const colW = panelW / cells.length;
  cells.forEach((cell, i) => {
    const cx = panelX + colW * i + colW / 2;
    if (i > 0) {
      doc.setDrawColor(panelBorder);
      doc.line(panelX + colW * i, panelY + 5, panelX + colW * i, panelY + panelH - 5);
    }
    doc.setFont('helvetica', 'normal');
    doc.setFontSize(7.5);
    doc.setTextColor(faint);
    doc.text(cell.label.toUpperCase(), cx, panelY + 11, { align: 'center', maxWidth: colW - 8 });
    doc.setFont('helvetica', 'bold');
    doc.setFontSize(13);
    doc.setTextColor(ink);
    doc.text(cell.value, cx, panelY + 21, { align: 'center', maxWidth: colW - 8 });
  });

  // --- Footer: disclaimer on the left, certificate ID + QR grouped
  // together on the right as one "verification" cluster - a larger,
  // easier-to-read certificate ID sits directly above the QR code and its
  // "Scan to verify" label, rather than a small line off in the opposite
  // corner. Laid out bottom-up from the inner border (pageHeight - 13) so
  // nothing collides or clips regardless of how many lines the wrapped
  // disclaimer takes.
  const qrSize = 24;
  const verifyColX = pageWidth - 20 - qrSize / 2;
  const footerRightEdge = pageWidth - 20;
  const qrY = pageHeight - 44;

  // The ID text sits on its own line above the QR, so it can use the full
  // width of the right-hand column (not just the QR's width) and is
  // right-aligned to the same margin as the QR/border - but a long ID can
  // still overrun the disclaimer on the left, so its font shrinks to fit
  // before that happens rather than clipping at the page edge.
  const idText = `Certificate ID: ${certificateId}`;
  const idMaxWidth = pageWidth - 40 - 90; // leaves the disclaimer's column clear
  doc.setFont('helvetica', 'bold');
  let idFontSize = 11;
  doc.setFontSize(idFontSize);
  while (idFontSize > 7 && doc.getTextWidth(idText) > idMaxWidth) {
    idFontSize -= 0.5;
    doc.setFontSize(idFontSize);
  }
  doc.setTextColor(ink);
  doc.text(idText, footerRightEdge, qrY - 6, { align: 'right' });

  doc.addImage(qrDataUrl, 'PNG', pageWidth - 20 - qrSize, qrY, qrSize, qrSize);
  doc.setFont('helvetica', 'normal');
  doc.setFontSize(7);
  doc.setTextColor(faint);
  doc.text('Scan to verify', verifyColX, qrY + qrSize + 5, { align: 'center' });
  ...
  doc.text(disclaimer, 20, pageHeight - 24, { align: 'left', maxWidth: pageWidth - 20 - qrSize - 32 });
